...
contract ContractTestHelper is IERC721Receiver {
    ...
    function pairCurrentTwice(
        ...
        uint c0 = XenoPair(pair).current(tokenIn, amountIn);
        XenoPair(pair).sync();
        uint c1 = XenoPair(pair).current(tokenIn, amountIn);
        return (c0, c1);
    }
    ...
    function hook(
        ...
        (address token0, address token1) = XenoPair(pair).tokens();
        if (amount0 != 0) {
    ...
    function multipleVeXenoCheckpoints(address veXeno) external {
        IVeXeno(veXeno).checkpointToken();
        IVeXeno(veXeno).checkpointToken();
        IVeXeno(veXeno).checkpointTotalSupply();
        IVeXeno(veXeno).checkpointTotalSupply();
    }
