...
contract XenoRouter01 {
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, "XenoRouter: EXPIRED");
        _;
    }
    ...
    receive() external payable {
        ...
        require(msg.sender == address(wmatic), "XenoRouter: NOT_WMATIC");
    }
    ...
    function _sortTokens(
        ...
        require(tokenA != tokenB, "XenoRouter: IDENTICAL_ADDRESSES");
        (token0, token1) = tokenA < tokenB
            ? (tokenA, tokenB)
            : (tokenB, tokenA);
        require(token0 != address(0), "XenoRouter: ZERO_ADDRESS");
    }
    ...
    function _quoteLiquidity(
        ...
        require(amountA > 0, "XenoRouter: INSUFFICIENT_AMOUNT");
        require(
            reserveA > 0 && reserveB > 0,
            "XenoRouter: INSUFFICIENT_LIQUIDITY"
        );
    ...
    function _getAmountsOut(
        ...
        require(routes.length >= 1, "XenoRouter: INVALID_PATH");
    ...
    function _addLiquidity(
        ...
        require(amountADesired >= amountAMin, "XenoRouter: DESIRED_A_AMOUNT");
        require(amountBDesired >= amountBMin, "XenoRouter: DESIRED_B_AMOUNT");
        // create the pair if it doesn't exist yet
        address _pair = IFactory(factory).getPair(tokenA, tokenB, stable);
        if (_pair == address(0)) {
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            if (amountBOptimal <= amountBDesired) {
                require(
                    ...
                    "XenoRouter: INSUFFICIENT_B_AMOUNT"
                );
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = _quoteLiquidity(
                ...
                require(
                    ...
                    "XenoRouter: INSUFFICIENT_A_AMOUNT"
                );
                (amountA, amountB) = (amountAOptimal, amountBDesired);
            }
        }
    ...
    function addLiquidityMATIC(
        ...
        returns (uint amountToken, uint amountMATIC, uint liquidity)
    {
        ...
        if (msg.value > amountMATIC)
            _safeTransferMATIC(msg.sender, msg.value - amountMATIC);
    }
    ...
    function _removeLiquidity(
        ...
        (amountA, amountB) = tokenA == token0
            ? (amount0, amount1)
            : (amount1, amount0);
        require(amountA >= amountAMin, "XenoRouter: INSUFFICIENT_A_AMOUNT");
        require(amountB >= amountBMin, "XenoRouter: INSUFFICIENT_B_AMOUNT");
    }
    ...
    function removeLiquidityMATICWithPermit(
        ...
        (amountToken, amountMATIC) = _removeLiquidityMATIC(
            token,
            stable,
            liquidity,
            amountTokenMin,
    ...
    function swapExactTokensForTokensSimple(
        ...
        require(
            ...
            "XenoRouter: INSUFFICIENT_OUTPUT_AMOUNT"
        );
    ...
    function swapExactMATICForTokens(
        ...
        require(routes[0].from == address(wmatic), "XenoRouter: INVALID_PATH");
    ...
    function swapExactTokensForMATIC(
        ...
        require(
            routes[routes.length - 1].to == address(wmatic),
            "XenoRouter: INVALID_PATH"
        );
        amounts = _getAmountsOut(amountIn, routes);
        require(
            amounts[amounts.length - 1] >= amountOutMin,
    ...
    function _safeTransferMATIC(address to, uint value) internal {
        ...
        require(success, "XenoRouter: ETH_TRANSFER_FAILED");
    }
}
