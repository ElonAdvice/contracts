...
contract XenoMinter is IMinter {
    ...
    constructor(
        ...
        address controller_ // controller with veXeno and voter addresses
    ) {
    ...
    function _veXeno() internal view returns (IVeXeno) {
        return IVeXeno(IController(controller).veXeno());
    }
    ...
    /// @dev Calculate circulating supply as total token supply - locked supply - veXeno balance - minter balance
    function circulatingSupply() public view returns (uint) {
        return
            token.totalSupply() -
            IUnderlying(address(ve)).totalSupply() -
            // exclude veXeno token balance from circulation - users unable to claim them without lock
            // late claim will lead to wrong circulation supply calculation
            token.balanceOf(address(_veXeno())) -
    ...
    function updatePeriod() external override {
        ...
        if (block.timestamp >= activePeriod && initializer == address(0)) {
            ...
            IVeXeno veXeno = _veXeno();
            IVoter voter = _voter();

            IERC20(address(token)).safeTransfer(address(veXeno), _growth);
            // checkpoint token balance that was just minted in veXeno
            veXeno.checkpointToken();
            // checkpoint supply
            veXeno.checkpointTotalSupply();
