...
describe("router tests", function () {
  ...
  before(async function () {
    ...
    factory = await Deploy.deployXenoFactory(owner);
    router = await Deploy.deployXenoRouter01(owner, factory.address, wmatic.address);
  ...
  it("receive matic not from wmatic reject", async function () {
    await expect(owner.sendTransaction({value: 1, to: router.address})).revertedWith("XenoRouter: NOT_WMATIC");
  });
